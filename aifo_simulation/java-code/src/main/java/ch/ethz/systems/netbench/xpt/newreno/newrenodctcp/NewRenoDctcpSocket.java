...
public class NewRenoDctcpSocket extends NewRenoTcpSocket {
    ...
    NewRenoDctcpSocket(TransportLayer transportLayer, long flowId, int sourceId, int destinationId, long flowSizeByte, int sourcePort, int destinationPort) {
    ...
    protected void processAcknowledgment(long ack, boolean isECE) {
        ...
        if (isECE) {
            // System.out.println("CW 1: " + congestionWindow);
            // System.out.println("-- alpha: " + alphaFraction);
            congestionWindow = Math.max(LOSS_WINDOW_SIZE, (1 - alphaFraction / 2.0) * congestionWindow);
            // System.out.println("-- CW 2: " + congestionWindow);
